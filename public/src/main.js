...
(async function bootstrap() {
  ...
  try {
    const [{ default: Router }] = await Promise.all([import('./router/Router.js')].map(p => p.catch(e => { throw e; })));

    // Lazy-load stores and services so they're not required unless main runs.
    const [
      { default: SettingsStore },
      { default: ReadStoriesStore },
      { default: StoryCommentThreadStore },
      { default: HNService }
    ] = await Promise.all([
      ...
      console.error('Failed to load core modules (stores/services). Ensure files exist under src/.', err);
      throw err;
    });
    ...
    const threadStore = new StoryCommentThreadStore();
    ...
    function lazyView(viewPath, opts = {}) {
      return async function viewFactory(params) {
        ...
        if (typeof ViewCtor !== 'function') {
          throw new Error(`View at ${viewPath} does not export a default constructor/function`);
        }
    ...
    router.register(/^#?\/?$/, lazyView('./views/ListView.js', { listType: 'top' }));
    router.register(/^#?\/newest$/, lazyView('./views/ListView.js', { listType: 'newest' }));
    router.register(/^#?\/ask$/, lazyView('./views/ListView.js', { listType: 'ask' }));
    router.register(/^#?\/show$/, lazyView('./views/ListView.js', { listType: 'show' }));
    router.register(/^#?\/jobs$/, lazyView('./views/ListView.js', { listType: 'jobs' }));
    ...
    router.setNotFound(async () => {
      ...
      const ViewCtor = module.default;
      return new ViewCtor({ params: {}, stores: { settingsStore, readStoriesStore, threadStore }, services: { hnService }, listType: 'top' });
    });
    ...
    if (appElement) {
      ...
      setTimeout(() => {
        try { appElement.focus(); } catch (e) { /* no-op */ }
      }, 120);
    }
  } catch (err) {
